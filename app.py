...
@app.route('/candidatos/<int:id_vaga>', methods=['POST', 'GET'])
def visualizar_candidatos(id_vaga):
    ...
    if request.method == 'GET':
        ...
        if not id_candidato:
            return "Erro: ID do candidato não fornecido.", 400
...
def pesquisar_vagas():
    ...
    try:
        conexao, cursor = conectar_db()
        comandoSQL = '''
        SELECT vaga.id_vaga, vaga.titulo, vaga.descricao, empresa.nome_empresa
        FROM vaga
        JOIN empresa ON vaga.id_empresa = empresa.id_empresa
        WHERE (vaga.titulo LIKE %s OR empresa.nome_empresa LIKE %s) and vaga.status = 'ativa';
        '''
        termo_pesquisa = f"{termo}%"
        cursor.execute(comandoSQL, (termo_pesquisa, termo_pesquisa))
        vagas = cursor.fetchall()

        return render_template('busca.html', vagas=vagas, termo=termo)
    except Error as erro:
        return f"ERRO! Erro de Banco de Dados: {erro}"
    except Exception as erro:
        return f"ERRO! Outros erros: {erro}"
